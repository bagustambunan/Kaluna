...
export function History() {
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Search + filter bar */}
      <div className="px-4 pt-4 pb-3 space-y-2 bg-stone-50 sticky top-0 z-10">
        <div className="flex gap-2">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2 text-sm bg-white border border-stone-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-stone-900"
            />
          </div>
          <button
            onClick={() => setFilterOpen(v => !v)}
            className={`px-3 py-2 rounded-lg border text-sm font-medium transition-colors ${filterOpen || hasFilters ? 'bg-stone-900 text-white border-stone-900' : 'bg-white border-stone-300 text-stone-600 hover:bg-stone-50'}`}
          >
        ...
        {filterOpen && (
          <div className="bg-white border border-stone-200 rounded-xl p-3 space-y-3">
            {/* Sort */}
            <div>
              <p className="text-xs font-medium text-stone-500 mb-1.5">Sort</p>
              <div className="flex flex-wrap gap-1.5">
                {(['newest','oldest','highest','lowest'] as SortKey[]).map(k => (
                  <button
                    key={k}
                    onClick={() => setSortKey(k)}
                    className={`px-3 py-1 text-xs rounded-lg capitalize border transition-colors ${sortKey === k ? 'bg-stone-900 text-white border-stone-900' : 'border-stone-200 text-stone-600 hover:bg-stone-50'}`}
                  >
                    {k}
                  </button>
                ))}
            ...
            {/* Categories */}
            <div>
              <p className="text-xs font-medium text-stone-500 mb-1.5">Category</p>
              <div className="flex flex-wrap gap-1.5">
                {state.categories.map(c => (
                  <button
                    key={c.id}
                    onClick={() => toggleCat(c.id)}
                    className={`px-2.5 py-1 text-xs rounded-lg border transition-colors ${selectedCats.has(c.id) ? 'text-white border-transparent' : 'border-stone-200 text-stone-600 hover:bg-stone-50'}`}
            ...
            {/* Amount range */}
            <div>
              <p className="text-xs font-medium text-stone-500 mb-1.5">Amount Range</p>
              <div className="flex items-center gap-2">
                <input
                  ...
                  placeholder="Min"
                  className="flex-1 px-2.5 py-1.5 text-xs border border-stone-300 rounded-lg focus:outline-none focus:ring-1 focus:ring-stone-900"
                />
