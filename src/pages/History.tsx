...
export function History() {
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Search + filter bar */}
      <div className="px-4 pt-4 pb-3 space-y-2 bg-paper sticky top-0 z-10 md:top-0">
        <div className="flex gap-2">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2 text-sm bg-sheet border border-ink/15 rounded-lg focus:outline-none focus:ring-2 focus:ring-pen text-ink"
            />
          </div>
          <button
        ...
        {filterOpen && (
          <div className="bg-sheet border border-ink/10 rounded-md p-3 space-y-3">
            {/* Sort */}
            <div>
              <p className="text-xs font-medium text-mute mb-1.5">Sort</p>
            ...
            {/* Categories */}
            <div>
              <p className="text-xs font-medium text-mute mb-1.5">Category</p>
              <div className="flex flex-wrap gap-1.5">
            ...
            {/* Amount range */}
            <div>
              <p className="text-xs font-medium text-mute mb-1.5">Amount Range</p>
            ...
            {hasFilters && (
              <button onClick={clearFilters} className="flex items-center gap-1 text-xs text-mute hover:text-ink">
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-4">
        {filtered.length === 0 ? (
          <p className="text-sm text-mute text-center py-12">
            {hasFilters ? copy.noMatch : copy.nothingRecorded}
          </p>
        ) : (
