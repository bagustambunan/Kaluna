    ...
    <div className="px-4 pt-5 pb-4 space-y-2">
      ...
      <SectionCard title="Preferences" open={openSection === 'preferences'} onToggle={() => toggle('preferences')}>
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm font-medium text-stone-700">Start of week</p>
            <p className="text-xs text-stone-400">Affects weekly summaries and budget calculations</p>
          </div>
          <div className="flex rounded-lg border border-stone-200 overflow-hidden">
            {(['monday','sunday'] as const).map(v => (
              <button
                ...
                className={`px-3 py-1.5 text-sm capitalize transition-colors ${state.settings.weekStartDay === v ? 'bg-stone-900 text-white' : 'text-stone-600 hover:bg-stone-50'}`}
              >
                {v}
              </button>
            ))}
      ...
      <SectionCard title="Budget" open={openSection === 'budget'} onToggle={() => toggle('budget')}>
        <div className="space-y-3">
          <div>
            <label className="block text-sm font-medium text-stone-700 mb-1">Weekly budget</label>
            <div className="relative">
              ...
              <input type="text" inputMode="numeric" value={weeklyBudget}
                ...
                placeholder="Not set"
                className="w-full pl-10 pr-3 py-2 text-sm border border-stone-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-stone-900"
              />
            </div>
          </div>
          <div>
            <label className="block text-sm font-medium text-stone-700 mb-1">Monthly budget</label>
            <div className="relative">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-stone-500 text-sm">Rp</span>
          ...
          <div>
            <label className="block text-sm font-medium text-stone-700 mb-1">Alert threshold</label>
            <div className="flex gap-2">
              {[50, 75, 90].map(v => (
                <button key={v}
                  onClick={() => dispatch({ type: 'UPDATE_BUDGETS', payload: { alertThresholdPct: v } })}
                  className={`flex-1 py-1.5 text-sm rounded-lg border transition-colors ${state.budgets.alertThresholdPct === v ? 'bg-stone-900 text-white border-stone-900' : 'border-stone-200 text-stone-600 hover:bg-stone-50'}`}
                >
                  {v}%
                </button>
              ))}
      ...
      <SectionCard title="Categories" open={openSection === 'categories'} onToggle={() => toggle('categories')}>
        <div className="space-y-3">
            ...
            <div key={cat.id} className="flex items-center gap-2">
              ...
              {editingCat?.id === cat.id ? (
                <input
                  ...
                  autoFocus
                  className="flex-1 px-2 py-1 text-sm border border-stone-300 rounded focus:outline-none focus:ring-1 focus:ring-stone-900"
                />
              ) : (
                <button onClick={() => setEditingCat(cat)} className="flex-1 text-left text-sm text-stone-700 hover:text-stone-900">
                  {cat.name}
                </button>
              )}
              {cat.isDefault ? (
          ...
          <div className="border-t border-stone-100 pt-3 space-y-2">
            <p className="text-xs font-medium text-stone-500">Add category</p>
            <input
              value={newCatName}
              onChange={e => setNewCatName(e.target.value)}
              placeholder="Category name"
              className="w-full px-3 py-2 text-sm border border-stone-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-stone-900"
            />
            <div>
              <p className="text-xs text-stone-500 mb-1">Emoji</p>
              <div className="flex flex-wrap gap-1.5">
                {EMOJI_OPTIONS.map(e => (
                  <button key={e} onClick={() => setNewCatEmoji(e)}
                    className={`text-lg p-1 rounded ${newCatEmoji === e ? 'ring-2 ring-stone-900' : ''}`}>
                    {e}
                  </button>
                ))}
              </div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {COLOR_OPTIONS.map(c => (
                  <button key={c} onClick={() => setNewCatColor(c)}
                    className={`w-6 h-6 rounded-full border-2 ${newCatColor === c ? 'border-stone-900' : 'border-transparent'}`}
      ...
      <SectionCard title="Shortcuts" open={openSection === 'shortcuts'} onToggle={() => toggle('shortcuts')}>
        {state.shortcuts.length === 0 ? (
          <p className="text-sm text-stone-400">No shortcuts yet. Save a transaction as a shortcut from the expense form.</p>
        ) : (
                ...
                return (
                  <div key={sc.id} className="flex items-center gap-2">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm text-stone-700 truncate">{sc.label}</p>
                      <p className="text-xs text-stone-400">{formatRupiah(sc.amount)} · {cat?.name}</p>
                    </div>
                    <button
                      onClick={() => dispatch({ type: 'DELETE_SHORTCUT', payload: sc.id })}
                      className="p-1 text-stone-400 hover:text-red-600 hover:bg-red-50 rounded shrink-0"
                    >
                      <Trash2 size={14} />
                    </button>
                  </div>
      ...
      <SectionCard title="Notifications" open={openSection === 'notifications'} onToggle={() => toggle('notifications')}>
        <div className="space-y-3">
          <div className="flex items-center justify-between text-sm">
            <span className="text-stone-700">Permission status</span>
            <span className={`font-medium ${notifPermission === 'granted' ? 'text-green-600' : notifPermission === 'denied' ? 'text-red-600' : 'text-stone-500'}`}>
          ...
          <p className="text-xs text-stone-400">Budget alert notifications are sent when you reach 75%, 100%, or exceed your budget.</p>
        </div>
      </SectionCard>
      ...
      <SectionCard title="Data" open={openSection === 'data'} onToggle={() => toggle('data')}>
        <div className="space-y-4">
          <div>
            <p className="text-sm font-medium text-stone-700 mb-1">Export data</p>
            <p className="text-xs text-stone-400 mb-2">Download all your data as a JSON file.</p>
            <Button onClick={handleExport} variant="secondary" size="sm">Export .json</Button>
          </div>

          <div>
            <p className="text-sm font-medium text-stone-700 mb-1">Import data</p>
            <p className="text-xs text-stone-400 mb-2">Upload a previously exported JSON file.</p>
              ...
              <div className="mt-3 bg-stone-50 border border-stone-200 rounded-xl p-3 space-y-3">
                <div className="text-sm space-y-1">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium text-stone-900">{importPreview.preview.count}</span>
                  </div>
                  {importPreview.preview.dateRange && (
                    <div className="flex justify-between">
                      <span className="text-stone-500">Date range</span>
                      <span className="font-medium text-stone-900 text-xs">{importPreview.preview.dateRange.start} – {importPreview.preview.dateRange.end}</span>
                    </div>
                  )}
                  <div className="flex justify-between">
                    <span className="text-stone-500">Total</span>
                    <span className="font-medium text-stone-900">{formatRupiah(importPreview.preview.total)}</span>
                  </div>
                </div>
                <div className="flex gap-2">
                ...
                <button onClick={() => setImportPreview(null)} className="text-xs text-stone-400 hover:text-stone-600">Cancel</button>
              </div>
            )}
          </div>

          <div className="border-t border-stone-100 pt-4">
            <p className="text-sm font-medium text-red-700 mb-1">Delete all data</p>
            <p className="text-xs text-stone-400 mb-2">This will permanently delete all expenses. Type DELETE to confirm.</p>
            <div className="flex gap-2">
              <input
                value={deleteConfirm}
                onChange={e => setDeleteConfirm(e.target.value)}
                placeholder="DELETE"
                className="flex-1 px-3 py-2 text-sm border border-stone-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-600"
              />
              <Button onClick={handleDeleteAll} variant="danger" size="sm" disabled={deleteConfirm !== 'DELETE'}>
                Delete
              </Button>
            </div>
          </div>
      ...
      <SectionCard title="About" open={openSection === 'about'} onToggle={() => toggle('about')}>
        <div className="space-y-2 text-sm text-stone-600">
          <div className="flex justify-between">
            <span>App</span>
            <span className="font-medium text-stone-900">Kaluna</span>
          </div>
          <div className="flex justify-between">
            <span>Version</span>
...
function SectionCard({ title, open, onToggle, children }: {
  ...
  return (
    <div className="bg-white rounded-xl border border-stone-200 overflow-hidden">
      <button onClick={onToggle} className="w-full flex items-center justify-between px-4 py-3 text-left hover:bg-stone-50">
